    ...
    viewer.addEventListener(
      ...
      (e) => {
        ...
        for (let i = models.length - 1; i >= 0; i--) {
          ...
          if (
            model.containsPoint &&
            model.containsPoint(new PIXI.Point(canvasX, canvasY))
          ) {
            const zoomFactor = e.deltaY > 0 ? 0.95 : 1.05;
            const newScale = Math.max(
              0.01,
              Math.min(2, model.scale.x * zoomFactor),
            );
...
class HeroModel {
  ...
  async create(src) {
    ...
    this._ParametersValues.eyeBlink = [
  ...
  pointerEventBind() {
    ...
    viewer.addEventListener("pointerdown", (e) => {
      if (this._Model.focusing) this._Model.focus(e.clientX, e.clientY);
    });
  }
...
$(document).ready(async () => {
  ...
  document.getElementById("addL2DModelBtn").onclick = async () => {
    ...
    if (l2dviewer.isModelInList(`${fulldata.charName}_${costdata.costumeName}`))
      return;
    ...
    settingBtn.onclick = () => {
      ...
      setupModelSetting(
        l2dviewer.findModel(`${fulldata.charName}_${costdata.costumeName}`),
      );
    };
